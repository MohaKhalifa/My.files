...
alias phtml='git add ~/git/Elgahid/* && git commit ~/git/Elgahid/* && cd ~/git/Elgahid && git push && cd - && sl'  # I'm wasting my time fr 
...
alias yaa='echo "Albums downloader, Links only" && read name && echo "Artist?" && read ight && echo "Album?" && read album &&  yt-dlp --yes-playlist -f 140 -P ~/Music/$ight/$album $name && exit' 
